    ...
    def simple_synchronous():
        ...
        while True:
            ...
            if live_camera:
                ...
                if videoOutput_depth and frameNumber % record_interval == 0:
                    print("Saving Depth Frame",frameNumber)
                    videoOutput_depth.write(depth_image)
...
def beginVideoRecording():
    ...
    videoOutput_depth = cv2.VideoWriter(gst_out, cv2.CAP_GSTREAMER, 0, float(framerate), (int(streamWidth), int(streamHeight)))
    if not videoOutput_depth.isOpened():
        print("Failed to open output")

    return videoOutput, videoOutput_depth
...
if __name__ == '__main__':
    ...
    try:
        ...
        if record_interval>0:
            videoOutput, videoOutput_depth = beginVideoRecording()
        ...
        simple_synchronous, synchronous_with_tracker,multiprocessing_with_tracker = setup(
            ...
            videoOutput_depth = videoOutput_depth
        )
        ...
        if videoOutput_depth:
            print("Saving Recorded Depth Video")
            videoOutput_depth.release()
            print("Finished Saving Depth Video")